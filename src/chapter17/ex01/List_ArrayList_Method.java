...
public class List_ArrayList_Method {
	...
	public static void main(String[] args) {
		/*
		 	List<E> : 인터페이스 , 메소드만 선언 되어 있다. 구현되어 있지 않다.
		...
		//2. add (int index, E element)  : index 방번에 값을 추가함. 
		aList.add(1, 6);     // 1 : index [방번호] , 6 [값] 
		System.out.println(aList);
		System.out.println(aList.size());    // List 컬렉션에 저장된 방의 갯수 
			//[3, 6, 4, 5]
		
		//3. addAll (다른 리스트객체) : 자신의 마지막 방에서 다른 컬레션 객체의 값을 모두 추가 
		...
		// 컬렉션 : List , Set, Map 
		// List의 값을 수정 : set() 
		//5. set(int index, E element) : index 방번호에 있는 값을 element로 들온 인자로 수정 
		...
		//7. remove (Object o ) : 방번호로 삭제하는것이 아니고 리스트에 저장된 값을 식별해서 삭제, 
		...
		//9. isEmpty() : 리스에 값이 존재하지 않을 경우 true , 존재하는 경우 false 
		...
		// 11. get(int index) : index 방번호에 값을 가져오기 
			...
			// 리스트의 값을 배열로 변환  
			//12. toArray()  : 리스트에 저장된 값을 배열로 변환 , 리턴 타입이 Object 이기 때문에 다운 캐스팅 해서 처리 
			 Object[] obj = aList2.toArray();  //

			 System.out.println(Arrays.toString(obj));
			
			//13. toArray(T[] t) :    리스트 ===> 배열  , T[]     <== 리스트를 배열로 끄집어 낼때 해당타입의 배열로 바로 끄집어 낸다.  
