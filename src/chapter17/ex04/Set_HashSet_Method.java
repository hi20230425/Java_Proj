...
public class Set_HashSet_Method {
	...
	public static void main(String[] args) {
		/*
		 	Set<E> : 인터페이스 , 객체화 할수 없다, 자식을 객체화 해서 타입으로는 지정 가능하다. 
		 		...
		 		- TreeSet<E> 		: 검색을 빠르게 하기 위해서   <=== 
		 				값을 저장할때 정렬해서 저장이됨  : 
		 					...
		 					내림 차순 정렬 ( DESC ) ( 10, 9 ==> 1 )  (Z, Y, X ===> A ) ( 하,카, ====>가 ) 
		 */
		...
		hSet1.add("가"); hSet1.add("나"); hSet1.add("가");    // "가"   <== Set 은 중복된 값을 저장할 수 없다. 
		...
		//8. Iterator 를 사용해서 set에 저장된 값을 출력    :    <== set은 방번호가 없어서 for 문을 사용해서 값을 가져올 수 없다. 
			// Iterator : Set에 저장된 값을 순회자 
				// hasNext() : set에 다음값이 존재하면 true, 다음값이 존재하지 않으면 false 
				// next() : 값을 출력하고, 다음값으로 이동 
		
		//8-1. Iterator 객체 생성 
		Iterator<String> iterator = hSet3.iterator();    //Set 객체에 Iterator 를 장착함. 
		...
		System.out.println("=====Set ==> Array 로 변환후 출력 ===========");
