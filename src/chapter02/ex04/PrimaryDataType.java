...
public class PrimaryDataType {
		...
		/*
		 자료형 : 
		    - 기본 자료형 : RAM STACK (스텍) 공간에 변수명, 변수의 값이 저장 
		    	...
		    	- 정수형 자료형의 기본은 : int         <== long 선언후 변수의 값 (리터럴)을 담을때 , l, L
	...
	//	boolean bool3 = 123; 		// 오류 발생 
		...
		// 상수 : 변수 이름 앞에 final 키가 붙은것, 상수명은 변수명과 구분 하기 위해서 대문자 사용
		final boolean BOOL4 = false; 
		System.out.println(BOOL4);   //false 
		
		// 상수는 값은 수정할 수 없다. 
		// BOOL4 = true ; 			// 오류발생 : 상수는 값을 수정 할 수 없다. 
		...
		// short : 2byte ( = 16bit ,  -2^15 ~ +2^15-1) 범위 의 값을 할당 할 수 있다. 
						//-32,768 ~ 32,767
		...
		//long : 8byte (= 64bit , -2^63 ~ + 2^63-1 ) 
			...
			// long 형으로 선언된 변수에 값을 할당 할때 주의 , 리터럴에 값을 넣을때 : L , l 
		...
		// 3. 실수형(소숫점)  자료형   : double 이 기본이여서, float 자료형에 값을 할당 할때, f, F 
		// 		-- float : 4byte,    소숫점 7자리까지 정밀도를 유지 
		//		-- double : 8byte,   소숫점 15자리까지 정밀도가 유지 
		
		float val10 = 1.123456789F ;    // 소숫점 이하 9자리까지넣었지만 F ( 7자리까지만 유지 ) 
		System.out.println(val10);		//1.1234568
		...
		// 4. 문자 (char) :  1글자  ,    문자열 : 1글자 이상 , String (참조 자료형 , 객체 )  , 리터럴에 값을 넣을때 ""
			...
			// -- 한글 1자, 중국어 1자 , 일본어 1자 . 
		// 직접 문자를 대입 : 영어 (대문자, 소문자), 숫자, 특수문자    <== 7bit 코드값 : 아스키 코드 
		// 유니코드 : 2byte ~3byte  , 한국어, 일본어, 중국어,...... 
		
	    char val12 = 'A';     // A 의 아스키 코드값 : 65
	    char val13 = 'B'; 
	    char val14 = 'a'; 
	    char val15 = '가'; 
	    char val16 = '#'; 
	    ...
	    //3. printf() : 문자열 내에서 ,  뒤의 변수 값를 불러들임 "%s" : 문자열, "%d": 정수 , "%5.2f" : 실수 
	    			// "%c" : 문자열을 변수값을 불러들임 
