...
public class Break_controll_Statement {
	public static void main(String[] args) {
		...
		System.out.println("===2. 다중(이중) Loop 문에서 break 사용  : 한 단계만 탈출===============");
		for ( int i = 0 ; i < 5 ; i++) {
			for ( int j = 0 ; j < 5 ; j++) {
		...
		// 3. 다중 (이중) Loop 문에서 점프해서 한번에 탈출 : LABLE 를 사용해서 점프 
		System.out.println("===3. 다중 (이중) Loop 문에서 점프해서 한번에 탈출 : LABLE 를 사용해서 점프=========");
		...
		for ( int i = 0 ; i < 5 ; i++) {
			for ( int j = 0 ; j < 5 ; j++) {
				...
				if (j == 2) {
					i = 50;     // 바깥쪽의 탈출할 조건을 false 로 만들어서 
					break ; 		//한 단계만 탈출  
				}
			}
		}
