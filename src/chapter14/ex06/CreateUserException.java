...
class A {
	...
	//정수를 인풋받아서 70 이하 이면 예외를 강제로 발생시킴 (예외를 직접 )
	void abc_1(int num) {
		...
		try {
			...
			System.out.println(e.getMessage());      //실행 예외가 발생 되었을때 예외 메세지를 출력 하라 
		}
	}
...
public class CreateUserException {
	...
	public static void main(String[] args) {
		/*
		 	사용자 정의 예외 : Java 에서 제공되지않는 예외를 자신이 직접 만들어서 사용하는 예외 
		 		- 일반 예외 (checked Exception) : 컴파일 단계에서 오류가 발생, 예외 처리가 반드시 필요
		 			- Exception 클래스를 상속 해서 사용자 정의 예외 (1. 기본 생성자, 2. 오류 메세지를 처리할 생성자) 
		 		- 실행 예외 (unchecked Exception , Runtime Exception) : 실행시 예외 발생. 
		 		    - RuntimeException 클래스를 상속 해서 사용자 정의 예외를 생성  (1. 기본 생성자, 2. 오류 메세지를 처리할 생성자)
