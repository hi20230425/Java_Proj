...
class A {  // 1. 생성자, 2. 필드, 3, 메소드 , 4. 이너클래스 
	...
	//필드 :
	   // - 인스턴스 필드 : 필드 이름 앞에 static 키가 들어가 있지 않은 필드 , Heap 에 저장됨 , 객체화 해야 사용가능 
				//다는 객체와 공유되지않는 필드, 
	   // - 정적(static) 필드 : 필드 이름 앞에 static 키가 붙어있는 필드 , static 공간에 저장됨. 
				//객체화 해서 사용, 객체화 하지 않고, 클래스 이름으로 바로 사용 ,
				//다른 객체화 공유해서 사용 가능 한 필드 
	
	int a ; 		//인스턴스(객체화) 필드 , 그 객체에서만 사용 가능 , Heap
	static int b ;  //정적(static) 필드, 모든 객체에서 공유 되는 필드 , RAM 클래스 영역에 저장됨  	
...
public class Field01 {
	public static void main(String[] args) {
		...
		//정적(static) 필드 : 객체화 하지 않고, 클래스이름으로 바로 사용가능 
		A.b = 50; 			// 클래스 이름으로 사용 
