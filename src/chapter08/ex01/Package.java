...
class A {				// class A는 접근 제어자가 default 이므로 외부 패키지에서는 접근이 되지 않음. 
	...
	public int a; 
	protected int b; 
	int c; 				//default  : 명시 해서 사용하지 않고, 생략시 default 가 생략되어있음. 
	private int d; 
...
public class Package {
	...
	public static void main(String[] args) {
		/*
		 	...
		 	클래스 외부 구성요소 : 1. 패키지 (package), 2. 임포트(import), 3. 외부 클래스 
		 		- 1. 패키지 : 클래스(파일)를 조직화 할때 사용, 폴더
		 				-- 같은 패키지에 동일한 이름의 클래스는 올수 없다.
		 		- 2. 임포트 (import)  : 
		 				-- 동일한 패키지에 존재 하지 않는 클래스는 import 를 사용해서 불러들여야 한다. 
		 				-- 외부의 클래스는 반드시 public class 이여야 불러들일 수 있다. 
		 		- 3. 외부 클래스 --> 클래스 외부에 생성된 클래스, public class는 하나만 와야 한다. 
		 		        -- public class가 아니면 외부에서 접근이 불가. 
		 		        
		     접근 제어자 : 정보를 보호하기 위해서 클래스 이름앞, 필드 이름앞, 메소드드 이름앞 붙는 제어자 
		       				변수 이름앞에는 붙지 않는다. <== 메소드 내부에서 선언된, 제어어문 내부에서 선언된..
		       		정보의 은닉 , 캡슐화  <== 보안...
		...
		// import 후 외부 패키지를 불러 들이 수 있음. 
		Student s1 = new Student(); 	//다르 패키지에 존재하는 클래스 : import를 사용해서 사용가능 
