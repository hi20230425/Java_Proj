...
// 보모 클래스 : 자식클래스의 공통적으로 적용된는 필드, 메소드 정의 
class Human {     //(부모)사람 <==> (자식) 학생, 교수, 근로자 
	...
	//메소드 : 인스턴스 메소드 
	void eat () {
		System.out.println("Human -  모든 사람은 잠은 잡니다. ");
	}
...
class Professor extends Human {
	...
	void goToTeach () {
		System.out.println("교수는 학들을 가르치러 학교에 갑니다. ");
	}
...
public class Ex01 {
	public static void main(String[] args) {
		...
		//Human 클래스 h1 객체 생성 :   h1 : Human 타입만 내포 하고 있음. 
		Human h1 = new Human();        
		   //h1 : Human의 필드와 메소드만 접근 
		   //h1 은 Human을 객체화 했기 때문에 Sudent / Professor / Worker 클래스를 내포하고 있지 않아서 다운 캐스팅 할수 업다. 
		...
		Student s1 = new Student();      //s1 : Human / Student 내포하고 있고, Student 타입 으로 지정되어 있다. 
			//s1 : Human의 필드와 메소드, Student 필드와 메소드를 모두 접근 가능 
		//s1은 Human의 필드 메소드, Student 필드 메소드 모두 접근 
		s1.name = "홍길동"; 		//Human
		s1.age = 20; 			//Human

		s1.stuID = 1234; 		//Student 에 필드 
		...
		//업캐스팅 : ss2 : Student (자식) ===> Human (부모)  : 명시 하지 않아도 자동으로 캐스팅됨 
		...
		//배열 / 컬렉션의 저장되는 것은 동일한 타입으로 지정 되어야 한다. 
		// s10 : Sudent , p10 : Professor , w10 : Worker    <== Human 
		...
		Professor s21 = (Professor) arr[1]; //Human ==> Professor 로 다운캐스팅 
		...
		Human h40 = new Student(); 			// h40 : Human / Stdent ,
		Human h41 = new Professor(); 
		Human h42 = new Worker(); 
		
		//Human 클래스의 eat() 메소드 호출
		h40.eat();       //Human클래스의 eat() 호출시, Stdent 클래스의 오버라이딩된 eat() 이 작동됨 
		h41.eat(); 		 //Human클래스의 eat() 호출시, Professor 클래스의 오버라이딩된 eat() 이 작동됨
		h42.eat(); 		//Human클래스의 eat() 호출시, Worker 클래스의 오버라이딩된 eat() 이 작동됨
