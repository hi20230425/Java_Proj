...
class Animal1 {
	...
	//인스턴스 메소드 : 오버라이딩은 인스턴스 메소드만 오버라이딩됨. <<인스터스 메소드 오버라이딩>> 
...
class Tiger1 extends Animal1  {
	@Override
	void run() {
		System.out.println("호랑이는 껑충 껑충달립니다. ");
	}
}
class Eagle1 extends Animal1 {
	@Override
	void run() {
		System.out.println("독수리는는 훨훨 나르니다. . ");
	}
}
class Cat1 extends Animal1 {
	@Override
	void run() {
		System.out.println("고양이는 사뿐사뿐달립니다. ");
	}
}
class CatChild1 extends Cat1 {
	@Override
	void run() {
		System.out.println("새끼고양이는는 엉금 업금 달립니다. ");
	}
}
...
public class Ex02 {
	...
	public static void main(String[] args) {
		...
		//Animal 의 run() 메소드   <==  자식 클래스의 run() 가 작동됨 
		a1.run(); 
		a2.run(); 
		a3.run(); 
		a4.run(); 
