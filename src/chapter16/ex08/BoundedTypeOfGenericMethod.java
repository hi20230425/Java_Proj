...
class A {   // 일반 클래스의 제너릭 메소드 
	...
	public <T extends Number > void method1 (T t) {   // 정수 나 실수만 처리하는 자료형만 올 수 있다. 
		
		System.out.println(t.intValue());    // t.intValue() : Number 타입의 메소드   
	}
}

//인터페이스 : 제너릭 타입 제한시 인터페이스일때 다중 제한이 가능 하다.  클래스 일대는 하나만 처리할 수 있다. 
		// < T extends IF1 & IF2 & IF3 > 
		// 인터이스로 제한을 두면 인터페이스를 구현한  하위의 모든 클래스가 타입으로 지정 될 수 있다. 
interface IF1 {
	void print() ; 		// public abstract 가 생략됨. 1. 자식 클래스에서 구현 클래스를 만들고 사용, 2. 익명 클래스를 사용해서 구현  
}
...
public class BoundedTypeOfGenericMethod {
	...
	public static void main(String[] args) {
		/*
		     제너릭 메소드에서 타입 제한  : 
		        - 타입 제한을 두지 않을때 : 모든 타입이 매개변수 인자로 오기 때문에 Object 의 메소드만 사용 가능 
